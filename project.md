...
| 2026-03-16 | **RSI/MACD UX cleanup.** Moved RSI/MACD toggles back to sidebar IndicatorToggle (they're indicators, not a separate UI category). Removed the separate tab button strip below the chart. Added corner labels to mini charts (RSI in purple, MACD in blue) for identification. Sidebar reverted to original clean form (pre-audit `dca2cad`). All charts use `autoSize: true` (lw-charts v5 recommended). Created `bugs.md` tracker and `left-bar-problems.md` audit doc. 298/298 tests, build clean. |
