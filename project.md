...
| 2026-03-17 | **Codebase cleanup.** Removed 3 dead backward-compat re-export files from Phase 12D (`useAlpacaBars.js`, `useAlpacaSocket.js`, `services/alpaca.js`). Removed stale compat aliases from `useBars.js` and `useLiveFeed.js`. Renamed `package.json` name `lumpia` → `lumpio`. Standardized localStorage keys from `lumpia-*` → `lumpio-*` prefix with migration fallback reads. Updated SettingsModal theme display name. |
