...
Lumpio gives 8 indicators, confluence scoring, one-click backtesting, and a trade
...
- Local dev + Vercel deployment (live at lumpio.space)
...
**Mobile (<768px):** Bottom navigation bar replaces TopNav panel toggles. Timeframe is a
single dropdown button (tap to select from all 6 timeframes), freeing space for all 4 panel
buttons (Alerts, Watchlist, Backtest, Journal) to appear directly — no "more" menu needed.
Right panels render inside a draggable `BottomSheet` component (snap points 50%/90%,
velocity-based dismiss). Sidebar opens as a fixed overlay. Safe areas respected via
`env(safe-area-inset-*)`. Landscape mode hides status bar and mini charts, compacts sub-header.
...
| **Mobile polish pass** | Touch targets 44px+ via `@media (pointer: coarse)`, chart fills viewport, sidebar opens via bottom nav hamburger button only |
...
**Performance targets:**
- Main bundle: <150KB (currently 147KB main + 161KB charts + 92KB motion + 69KB vendor + 183KB shared-react, lazy chunks — Vite 8/Rolldown)
...
| `src/components/layout/BottomNav.jsx` | Mobile-only bottom navigation — timeframe dropdown, all 4 panel buttons, sidebar hamburger |
...
| `src/components/ui/StatusBar.jsx` | WebSocket/Polling status + last updated time + session stats + Stripe donate |
...
| `src/roadmap-main.jsx` | Roadmap JS entry — minimal shell (back link, no stores/query/theme sync) |
| `src/roadmap.css` | Standalone roadmap stylesheet — fixed dark color scheme, independent from main app themes |
...
- [x] Phase 6: Vercel deployment (serverless proxy, lumpio.space, SSL)
...
- [x] Phase 11B: CSS theme refactor (33 !important eliminated), touch targets 44px+, chart snapshot
...
- [x] Phase 13A: Traction readiness — snapshot watermarks, welcome banner, Stripe donate link, branded OG image
...
- [x] Phase 14A: Chart visual overhaul — Webull-inspired (dark navy bg, fatter candles, 2px EMAs, visible volume, dotted grid, dashed crosshair)
...
- [x] Roadmap enhancements — curated beginner path filter (P1–P5 + P10 Risk Management + P11 Psychology), estimated time per phase, mastered badges, footer disclaimer, content accuracy fixes (T+1 settlement, sourced loss stats, ICT/SMC balance note)
- [x] Roadmap CSS decoupling — own standalone stylesheet (`roadmap.css`), fixed dark color scheme independent from main app themes
- [x] Security hardening — CSP (base-uri, form-action, object-src, upgrade-insecure-requests), COOP/CORP headers, expanded Permissions-Policy, X-XSS-Protection, fixed Vercel header routing (`/:path*`), fetch timeouts, crypto.randomUUID(), CI audit + lockfile-lint, Dependabot
- [x] Timezone fix — `toETTime()` normalizes ICU hour 24 → 0 for midnight ET across platforms
- [x] Chart stability — fixed preset switching crashes (overlay effect try/catch + disposedRef, mini chart crosshair handler race, CandlestickChart prepend null guard)
...
| 2026-03-13 | Vercel deployment: serverless proxy (api/bars.js) for API key security, removed VITE_ prefix from env vars, verified zero keys in browser bundle. Custom domain lumpio.space via CNAME. SSL cert propagating. |
...
| 2026-03-15 | Phase 11: Security + Polish + PWA. **11A:** localStorage schema validation (`validate.js` + 29 tests), wired into presets/journal/watchlist/symbol stores. API error sanitization (bars.js + snapshot.js never leak upstream status). Notification API guard. Code splitting — React.lazy for 6 components, manualChunks for lightweight-charts (501KB → 303KB main + 164KB charts). CSP updated for PWA. **11B:** Eliminated 33 CSS !important overrides → 16 semantic utility classes. Touch targets 44px+ via `@media (pointer: coarse)`. Chart snapshot (`snapshot.js` + Cmd+Shift+S + command palette). **11C:** PWA manifest + network-first service worker + icons. Onboarding tour (4-step tooltip, mobile welcome toast). Updated all docs. 257/257 tests, build clean. |
...
| 2026-03-17 | **Codebase cleanup.** Removed 3 dead backward-compat re-export files from Phase 12D (`useAlpacaBars.js`, `useAlpacaSocket.js`, `services/alpaca.js`). Removed stale compat aliases from `useBars.js` and `useLiveFeed.js`. Renamed `package.json` name `lumpio` → `lumpio`. Standardized localStorage keys from `lumpio-*` → `lumpio-*` prefix with migration fallback reads. Updated SettingsModal theme display name. |
...
| 2026-03-17 | **Phase 14C complete: Chart interaction UX.** Modernized chart scroll/interaction behavior in `CandlestickChart.jsx`. Added kinetic scrolling (`kineticScroll: { touch: true, mouse: true }`) for momentum/inertia on drag-release — feels like scrolling on a phone or modern web app. Switched crosshair to magnet mode (`CrosshairMode.Magnet`) — snaps to nearest OHLC value for precision instead of floating freely. Added explicit `handleScroll` config with `vertTouchDrag: false` to prevent accidental vertical scrolls during horizontal chart panning on mobile. Added `handleScale` with `axisDoubleClickReset: true` for quick zoom reset. 298/298 tests, build clean, ESLint 0 errors. |
...
| 2026-03-24 | **Trading Roadmap page.** Standalone `/roadmap` at separate Vite entry point (`roadmap.html` + `src/roadmap-main.jsx`). 16 phases, 86 topics, 500+ concepts. Interactive learning tracker with progress via localStorage. Detail panel (desktop side panel / mobile bottom sheet overlay). Search across topics, concepts, resources. Data in `src/constants/roadmap.js`. Linked from TopNav "Learn" button. Vercel rewrite `/roadmap` → `/roadmap.html`. **Enhancements:** Curated beginner path filter (P1–P5 + P10 Risk Mgmt + P11 Psychology, persisted), estimated time per phase, "Mastered" badge on phase completion, legal disclaimer footer. Content fixes: T+1 settlement corrected, loss stats sourced (ESMA/FINRA), ICT/SMC balance note. 298/298 tests, build clean. |
| 2026-03-24 | **Roadmap CSS decoupling.** Created `src/roadmap.css` — standalone stylesheet with fixed dark color scheme. Roadmap no longer imports `index.css` or inherits the main app's theme system (dark/terminal/lumpio). Removed theme/accent syncing from `roadmap-main.jsx`. Documentation synced across CLAUDE.md, project.md, tasks.md, security.md, memory files. All 87 documented files verified present on disk. 298/298 tests, build clean. |
...
| 2026-03-25 | **Visual polish — Webull-level chart tuning.** Compared Lumpio vs Webull mobile screenshots. 6 changes: (1) chart bg `#0a0a0a` → `#0b1018` dark navy for richer feel, (2) EMA line width 1px → 2px for all periods (was 2px only for 200), (3) volume bar opacity 33% → 60% (`#22c55e55` → `#22c55e99`), (4) volume height 18% → 25% of chart (`scaleMargins.top: 0.82` → `0.75`), (5) bar spacing 8 → 10, min 2 → 3 for fatter candles, (6) CrosshairLegend 10px → 11px with roomier padding. App shell bg synced across dark theme CSS vars. 298/298 tests, build clean. |
| 2026-03-25 | **Chart stability — preset switch smoothness.** Deep-dived the re-render cascade that caused chart flicker/zoom issues during preset switches. 5 fixes: (1) **Batched preset application** — `applyPreset()` now uses single `useChartStore.setState()` for indicators + timeframe instead of two separate calls (eliminates multi-wave re-render). (2) **LevelOverlay rewrite** — ODC LineSeries now created once on mount and updated via `setData()` (was tearing down and rebuilding all series on every bar change). (3) **Callback ref for chart detection** — replaced `setInterval` polling in App.jsx with `useCallback` ref for deterministic, immediate chart instance detection. (4) **Transition guard** — overlays receive `stableBars` (null during `isPlaceholderData`) to prevent thrashing with stale keepPreviousData during timeframe transitions. (5) **Mini charts always mounted** — IndicatorTabView uses CSS `height: 0` transition instead of conditional rendering, avoiding expensive `createChart()` mount/unmount cycles. 298/298 tests, build clean. |
| 2026-03-25 | **Mobile BottomNav UX overhaul.** Replaced scrollable timeframe pills + "more" popover menu with a compact timeframe dropdown button + direct panel buttons. Layout: `[☰] [5m ▾] —spacer— [🔔] [📋] [📊] [📓]`. Timeframe is a single button showing active TF with chevron — tap opens upward popover with all 6 options. Backtest + Journal buttons promoted from hidden "..." menu to always-visible icon buttons with per-theme colors. Removed `MORE_PANELS` constant and `moreOpen` state. Roadmap link removed from BottomNav (accessible via TopNav "Learn" button). 298/298 tests, build clean. |
| 2026-03-25 | **Roadmap visual overhaul.** Complete redesign of `/roadmap` page aesthetics. (1) **Color scheme:** replaced cold blue/black with deep indigo base (`#0c0915`), teal (`#00d4aa`), orange (`#ff8c32`), and rose (`#ff6b8a`) accent palette — vibrant, modern, alive. (2) **Hero section:** mesh gradient glow backdrop (3 radial gradients), gradient text title (teal→orange→rose), colorful stat numbers, glass-feel search with glow-on-focus, taller layout with breathing room. (3) **Cards:** glass gradient backgrounds, hover lifts 3px with layered shadows + inset highlight, active glow radiates outward, readable subtitle text. (4) **Header:** `[< Back] | [LUMPIO BETA] ——— Trading Roadmap` with teal→orange gradient title, responsive padding (`px-4 md:px-8`). (5) **Progress bar** moved from separate header row into hero content. Own standalone color scheme — fully independent from main app themes. |
